...
function Timesheets({user,employees,projects,entries,setEntries,timesheets,setTimesheets,setView}){
  const [week,setWeek]=useState(currentWeek());
  const [localEntries,setLocalEntries]=useState([]); // [{tempId,id,day,projId,desc,hours}]
  const [saving,setSaving]=useState(false);const [submitting,setSubmitting]=useState(false);
  ...
  // Reload local entries whenever week or DB entries change
  useEffect(()=>{
    const existing=entries.filter(e=>String(e.empId)===String(user.employeeId)&&e.week===week);
    setLocalEntries(existing.map(e=>({tempId:e.id||("t"+Date.now()+Math.random()),id:e.id,day:e.day||DAYS[0],projId:String(e.projId||""),desc:e.note||"",hours:String(e.hours||"")})));
  },[week,user.employeeId]);

  const addEntry=day=>setLocalEntries(prev=>[...prev,{tempId:"n"+Date.now()+Math.random(),id:null,day,projId:"",desc:"",hours:""}]);
  const upd=(tempId,field,val)=>setLocalEntries(prev=>prev.map(e=>e.tempId===tempId?{...e,[field]:val}:e));
  const del=tempId=>setLocalEntries(prev=>prev.filter(e=>e.tempId!==tempId));

  const totalHrs=localEntries.reduce((s,e)=>s+(+e.hours||0),0);
  const dayTotal=day=>localEntries.filter(e=>e.day===day).reduce((s,e)=>s+(+e.hours||0),0);
  const utilPct=capacity>0?Math.round((totalHrs/capacity)*100):0;
  const stMeta=TS_STATUS[ts?.status||"draft"];

  const saveDraft=async()=>{
    if(!user.employeeId){setMsg({type:"error",text:"No employee profile linked. Ask your admin."});return;}
    setSaving(true);setMsg({type:"",text:""});
    const valid=localEntries.filter(e=>e.projId&&+e.hours>0);
    const totalH=valid.reduce((s,e)=>s+(+e.hours),0);
    ...
    if(valid.length>0){const{data:newE}=await sb.from("time_entries").insert(valid.map(e=>({employee_id:user.employeeId,project_id:e.projId,week,day:e.day,hours:+e.hours,note:e.desc,timesheet_id:tsData.id}))).select();if(newE)setEntries(prev=>[...prev.filter(e=>!(String(e.empId)===String(user.employeeId)&&e.week===week)),...newE.map(toEntry)]);}
  ...
  const submit=async()=>{
    const valid=localEntries.filter(e=>e.projId&&+e.hours>0);
    if(!valid.length){setMsg({type:"warn",text:"Add at least one entry before submitting."});return;}
    ...
    if(data){setTimesheets(prev=>prev.map(t=>t.empId===user.employeeId&&t.week===week?toTs(data):t));setMsg({type:"ok",text:"Timesheet submitted!"});const myEmp=employees.find(e=>e.id===user.employeeId);if(myEmp?.managerId)await notifyEmp(myEmp.managerId,(user.name||"Someone")+" submitted timesheet for "+weekLabel(week),"timesheet");}
    setSubmitting(false);
  };
  const exportTS=()=>{
    const rows=localEntries.map(e=>{const p=projects.find(pr=>String(pr.id)===e.projId);return{Week:weekLabel(week),Day:e.day,Project:p?.name||"",Task_Description:e.desc,Hours:e.hours};});
    if(!rows.length){setMsg({type:"warn",text:"No entries to export."});return;}
    csvDownload(rows,"timesheet-"+week+".csv");
  };

  return(
    <div style={{fontFamily:FONT}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div><h1 style={{fontSize:21,fontWeight:700,color:TEXT,margin:"0 0 3px"}}>My Timesheet</h1><p style={{color:MUTED,fontSize:13,margin:0}}>Log individual tasks against projects each day</p></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"240px 1fr",gap:16}}>
        ...
        <div>
          <Card style={{marginBottom:12,padding:16}}>
            <div style={{fontSize:11,fontWeight:600,color:MUTED,textTransform:"uppercase",letterSpacing:.5,marginBottom:8}}>Week</div>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
              <button onClick={()=>setWeek(addWeeks(week,-1))} style={{width:30,height:30,borderRadius:6,border:"1px solid "+BORDER,background:WHITE,cursor:"pointer",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",color:TEXT}}>{"<"}</button>
              <div style={{flex:1,textAlign:"center"}}>
                <div style={{fontSize:12,fontWeight:700,color:TEXT,lineHeight:1.3}}>{weekLabel(week)}</div>
                <div style={{fontSize:10,color:week===cw?BLUE:MUTED,fontWeight:600,marginTop:2}}>{week===cw?"Current week":week>cw?"Future":"Past"}</div>
              </div>
              <button onClick={()=>setWeek(addWeeks(week,1))} style={{width:30,height:30,borderRadius:6,border:"1px solid "+BORDER,background:WHITE,cursor:"pointer",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",color:TEXT}}>{">"}</button>
            </div>
            {week!==cw&&<button onClick={()=>setWeek(cw)} style={{width:"100%",padding:"5px",background:"#EFF6FF",border:"1px solid "+BLUE+"44",borderRadius:6,color:BLUE,fontSize:11,fontWeight:600,cursor:"pointer",marginBottom:8,fontFamily:FONT}}>Jump to current week</button>}
            <div style={{fontSize:10,fontWeight:600,color:MUTED,textTransform:"uppercase",letterSpacing:.4,marginBottom:4}}>Quick select</div>
            {[addWeeks(cw,-3),addWeeks(cw,-2),addWeeks(cw,-1),cw].map(w=>{const wts=timesheets.find(t=>t.empId===user.employeeId&&t.week===w);const wst=wts?.status||"none";return(
              <div key={w} onClick={()=>setWeek(w)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 7px",borderRadius:5,cursor:"pointer",background:week===w?"#EFF6FF":WHITE,border:"1px solid "+(week===w?BLUE+"55":BORDER),marginBottom:3}}>
                <span style={{fontSize:11,fontWeight:week===w?600:400,color:week===w?BLUE:TEXT}}>{weekLabel(w)}</span>
                {wst!=="none"?<span style={{fontSize:9,background:TS_STATUS[wst]?.bg,color:TS_STATUS[wst]?.fg,borderRadius:4,padding:"1px 5px",fontWeight:600}}>{TS_STATUS[wst]?.label}</span>:<span style={{fontSize:9,color:BORDER}}>-</span>}
              </div>
            );})}
          </Card>
          {emp&&<Card style={{padding:14}}>
            <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:10}}>
              <Av name={emp.name} color={emp.color||BLUE} sz={32}/>
              <div><div style={{fontSize:13,fontWeight:600,color:TEXT}}>{emp.name}</div><div style={{fontSize:11,color:MUTED}}>{emp.role}</div></div>
            </div>
            <div style={{fontSize:12,color:MUTED,marginBottom:4}}>{totalHrs}h logged / {capacity}h capacity</div>
            <Prog val={utilPct} h={6}/>
            <div style={{fontSize:12,fontWeight:700,color:utilColor(utilPct).fg,marginTop:4,textAlign:"right"}}>{utilPct}%</div>
          </Card>}
        </div>

        {/* ── Day entry cards ── */}
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"9px 14px",background:stMeta?.bg||"#F8FAFC",borderRadius:8,marginBottom:12,border:"1px solid "+BORDER}}>
          ...
          {wkDates.map(dt=>{
            ...
            const dayEntries=localEntries.filter(e=>e.day===day);
            const dtotal=dayTotal(day);
            const isWeekend=(dt.getDay()===0||dt.getDay()===6);
            return(
              <Card key={day} style={{marginBottom:10,padding:0,border:"1px solid "+(isWeekend?"#F1F5F9":BORDER)}}>
                {/* Day header */}
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",background:isWeekend?"#FAFAFA":"#F8FAFF",borderRadius:"9px 9px 0 0",borderBottom:"1px solid "+BORDER}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{width:32,height:32,borderRadius:8,background:dtotal>0?BLUE:"#E2E8F0",color:dtotal>0?"#fff":MUTED,fontWeight:700,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}>{dt.getDate()}</div>
                    <div>
                      <div style={{fontSize:13,fontWeight:700,color:TEXT}}>{DAYS[(dt.getDay()+6)%7]}</div>
                      <div style={{fontSize:11,color:MUTED}}>{MONTHS[dt.getMonth()]} {dt.getDate()}{isWeekend?" (Weekend)":""}</div>
                    </div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    {dtotal>0&&<span style={{fontSize:14,fontWeight:700,color:BLUE}}>{dtotal}h</span>}
                    {!locked&&<button onClick={()=>addEntry(day)} style={{padding:"5px 12px",border:"1px solid "+BLUE+"55",borderRadius:6,background:"#EFF6FF",color:BLUE,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:FONT}}>+ Add Entry</button>}
                  </div>
                </div>

                {/* Entries */}
                <div style={{padding:dayEntries.length>0?"10px 14px":"0"}}>
                  {dayEntries.length===0&&!locked&&<div style={{padding:"8px 14px",fontSize:12,color:MUTED,fontStyle:"italic"}}>No entries - click + Add Entry to log time</div>}
                  {dayEntries.length===0&&locked&&<div style={{padding:"8px 14px",fontSize:12,color:MUTED}}>No hours logged</div>}
                  {dayEntries.map((entry,ei)=>{
                    const proj=projects.find(p=>String(p.id)===entry.projId);
                    return(
                      <div key={entry.tempId} style={{display:"grid",gridTemplateColumns:"190px 1fr 72px 36px",gap:8,alignItems:"center",padding:"8px 0",borderBottom:ei<dayEntries.length-1?"1px solid #F1F5F9":"none"}}>
                        {/* Project selector */}
                        {locked
                          ?<div style={{fontSize:13,fontWeight:500,color:TEXT,padding:"6px 0"}}>{proj?.name||"-"}</div>
                          :<select value={entry.projId} onChange={e=>upd(entry.tempId,"projId",e.target.value)} style={{padding:"7px 8px",border:"1.5px solid "+(entry.projId?BLUE:BORDER),borderRadius:6,fontSize:12,color:entry.projId?TEXT:MUTED,fontFamily:FONT,background:WHITE,width:"100%"}}>
                        ...
                        {/* Task / description */}
                        {locked
                          ?<div style={{fontSize:13,color:TEXT,padding:"6px 0"}}>{entry.desc||<span style={{color:MUTED,fontStyle:"italic"}}>No description</span>}</div>
                          :<input value={entry.desc} onChange={e=>upd(entry.tempId,"desc",e.target.value)} placeholder="Task / ticket e.g. JIRA-220, Meeting, Code review..." style={{padding:"7px 10px",border:"1px solid "+BORDER,borderRadius:6,fontSize:12,color:TEXT,fontFamily:FONT,width:"100%",boxSizing:"border-box"}}
                            onFocus={e=>e.target.style.borderColor=BLUE} onBlur={e=>e.target.style.borderColor=BORDER}/>
                        }
                        {/* Hours */}
                        {locked
                          ?<div style={{textAlign:"center",fontWeight:700,fontSize:14,color:+entry.hours>0?BLUE:MUTED}}>{entry.hours||"0"}h</div>
                          :<input type="number" min="0" max="24" step="0.5" value={entry.hours} onChange={e=>upd(entry.tempId,"hours",e.target.value)} placeholder="hrs" style={{padding:"7px 6px",border:"1.5px solid "+(+entry.hours>0?BLUE:BORDER),borderRadius:6,fontSize:14,fontWeight:700,textAlign:"center",width:"100%",boxSizing:"border-box",color:+entry.hours>0?BLUE:TEXT,fontFamily:FONT,background:+entry.hours>0?"#EFF6FF":WHITE}}/>
                        }
                        {/* Delete */}
                        {!locked
                          ?<button onClick={()=>del(entry.tempId)} title="Remove" style={{width:32,height:32,border:"none",background:"#FEF2F2",color:DANGER,borderRadius:6,cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>x</button>
                          :<div/>
                        }
                      </div>
                    );
          ...
          <Card style={{marginTop:4}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
                <div style={{fontSize:11,color:MUTED,fontWeight:600,textTransform:"uppercase",letterSpacing:.4,marginBottom:3}}>Week Total</div>
                <div style={{display:"flex",alignItems:"baseline",gap:8}}>
                  <span style={{fontSize:28,fontWeight:700,color:totalHrs>capacity?DANGER:BLUE}}>{totalHrs}h</span>
                  <span style={{fontSize:13,color:MUTED}}>of {capacity}h capacity ({utilPct}%)</span>
                </div>
                <div style={{marginTop:6,width:200}}><Prog val={utilPct} h={6}/></div>
              </div>
              {!locked&&user.employeeId&&<div style={{display:"flex",gap:10}}>
                <Btn disabled={saving} onClick={saveDraft}>{saving?<><Spin dark/>Saving...</>:"Save Draft"}</Btn>
                <Btn primary disabled={submitting||saving} onClick={submit}>{submitting?<><Spin/>Submitting...":"Submit for Approval"}</Btn>
              </div>}
              {locked&&<div style={{padding:"10px 16px",background:stMeta?.bg,borderRadius:8,fontSize:13,fontWeight:600,color:stMeta?.fg}}>{stMeta?.icon} {stMeta?.label} - awaiting review</div>}
            </div>
          </Card>
        </div>
      </div>
